...
`,
	...
	const settings = {
		...
		"workbench.view.extension.ashutoshshrimal.scaler-companion-ActivityBar.visible": true,
	...
	const keybindings = [
		{
			key: "alt+c",
			command: "workbench.view.extension.ashutoshshrimal.scaler-companion-ActivityBar",
			when: "viewContainer.workbench.view.extension.ashutoshshrimal.scaler-companion-ActivityBar.enabled",
		},
		{
			key: "alt+shift+c",
	...
	const args = [
		...
		"--start-up-extension",
		"ashutoshshrimal.scaler-companion",
		// Run a command on startup to open Cline
		"--command",
		"workbench.view.extension.ashutoshshrimal.scaler-companion-ActivityBar",
	...
	const startupScript = `
		...
		setTimeout(() => {
			...
			require('vscode').commands.executeCommand('workbench.view.extension.ashutoshshrimal.scaler-companion-ActivityBar');
			...
			function activate(context) {
				...
				let disposable = vscode.commands.registerCommand('cline-activator.activate', async function () {
					try {
						...
						const extension = vscode.extensions.getExtension('ashutoshshrimal.scaler-companion');
						if (!extension) {
						...
						await vscode.commands.executeCommand('workbench.view.extension.ashutoshshrimal.scaler-companion-ActivityBar');
